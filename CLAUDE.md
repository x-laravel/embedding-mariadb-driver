...
| `MariadbEmbeddingServiceProvider.php` | `register()` binds `VectorStore` → `MariadbVectorStore`. `boot()` registers `mariadb` similarity driver, adds `VEC_ToText` global scope to `Embedding` model, loads migration, publishes under `embedding-mariadb-migrations` tag. |
