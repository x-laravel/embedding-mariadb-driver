...
use XLaravel\Embedding\Driver\Mariadb\Tests\Fixtures\Models\VenueWithPayload;
use XLaravel\Embedding\Driver\Mariadb\Tests\TestCase;
use XLaravel\Embedding\Models\Embeddable as EmbeddableRecord;

class MariadbPayloadFilterTest extends TestCase
{
    ...
    public function test_float_filter_matches_integer_payload_value(): void
    {
        $this->assertSame(
            [$this->alpha->id, $this->gamma->id],
            $this->search(['province_id' => 34.0]),
        );
    }
    ...
    public function test_null_payload_value_never_matches_a_scalar_filter(): void
    {
    ...
    private function setVector(VenueWithPayload $venue, array $vector, string $slot = 'default'): void
    {
        DB::connection(config('embedding.database.connection'))->statement(
            'UPDATE `'.config('embedding.database.embeddings_table', 'embeddings').'` SET `vector` = Vec_FromText(?), updated_at = NOW() WHERE embeddable_type = ? AND embeddable_id = ? AND slot = ?',
